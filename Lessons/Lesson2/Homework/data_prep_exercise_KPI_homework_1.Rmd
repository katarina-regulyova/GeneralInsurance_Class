...
```{r}
BusinessColNumber = 4
dt_KPI_raw[is.na(dt_KPI_raw[,BusinessColNumber]), BusinessColNumber] = names(sort(table(dt_KPI_raw$Business),decreasing=TRUE)[1])

```
##### What is the feasible way to repair values that don't make sense? (e.g Can we see negative Premium?)
```{r}
mean(dt_KPI_raw$Premium)
PremiumColNumber = 8
dt_KPI_raw[dt_KPI_raw[,PremiumColNumber] < 0, PremiumColNumber] <- mean(dt_KPI_raw[,PremiumColNumber])
mean(dt_KPI_raw$Premium)
```
...
```{r}
# Probably there is something... and maybe not.
```
